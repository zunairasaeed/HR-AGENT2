...
def write_tailored_docx_from_json(tailored_json_path, docx_template_path, output_dir):
    ...
    for key, value in data.items():
        if key in ["CANDIDATE_NAME", "TARGET_JOB", "TEMPLATE_USED", "GPT_FILLED", "TAILORING_SCORE"]:
            continue
        if isinstance(value, list):
            value = "- " + "\n- ".join(value)

        if key.lower() == "personal info":
            insert_personal_info_top(doc, value)
        else:
            replace_docx_section_content(doc, key, str(value))

    fname = f"{data['CANDIDATE_NAME'].lower().replace(' ', '_')}__{data['TARGET_JOB'].lower().replace(' ', '_')}__tailored.docx"
    save_path = os.path.join(output_dir, fname)
    doc.save(save_path)
    print(f"📄 Saved final tailored DOCX → {save_path}")
...
#     # Step 1: Load the section structure from JSON template meta
#     sections, template_name = load_template_structure()

#     # Step 2: Generate tailored JSON content for the candidate
#     tailor_candidate_to_job(candidate_name, job_title, sections, template_name)

#     # Step 3: Construct path to tailored JSON file
#     json_filename = f"{candidate_name.lower().replace(' ', '_')}__{job_title.lower().replace(' ', '_')}__tailored.json"
#     tailored_json_path = os.path.join(OUTPUT_DIR, json_filename)

#     # Step 4: Path to DOCX template
#     docx_template_path = "templates/Word Template.docx"  # ✅ Ensure this exists

#     # Step 5: Generate the final tailored Word doc
